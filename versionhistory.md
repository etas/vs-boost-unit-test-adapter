...
- Created a __Boost External Test Runner__ so that users can execute Boost Unit test project compiled as a dynamic link library.
...
- When installing the Boost Unit Test Adapter directly from Visual Studio (go to __Tools__ -> __Extensions and Updates__-> __Online__ and search for term __Boost__), the Unit test adapter gets installed without any error.
